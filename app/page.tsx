...
const CATS: { icon: IconName; name: string; desc: string }[] = [
    { icon: "bot", name: "AI", desc: "GPT, image generator, translate, code & 20+ tugas AI." },
    { icon: "download", name: "Downloader", desc: "TikTok, YouTube, Instagram, Facebook, Spotify & lainnya." },
    { icon: "search", name: "Search", desc: "YouTube, web, lyrics, github, npm, anime." },
    { icon: "wrench", name: "Utility", desc: "QR, IP, weather, base64, password, hash." },
    { icon: "image", name: "Image", desc: "Remove background, upscale, resize, meme maker." },
    { icon: "text", name: "Text", desc: "OCR, summarize, grammar, TTS, rewrite." }
]
    ...
    return (
        <PageTransition>
            ...
            <section className="relative px-4 pt-36 pb-20 text-center">
                ...
                {/* Live stats — SSR (data server-first, counter dianimasikan di client) */}
                        ...
                        <StaggerItem key={i}>
                            <TiltCard className="glass noise rounded-2xl p-5">
                                <div className="font-mono text-2xl font-bold md:text-3xl">
                                    {s.isFloat ? <Counter value={s.n} decimals={2} /> : <Counter value={s.n} />}
                                    {s.suffix || ""}
            ...
            <section className="px-4 py-16">
                <div className="mx-auto max-w-6xl">
                    <Reveal className="mb-12 text-center">
                        ...
                        <h2 className="mt-2 text-3xl font-bold md:text-4xl">Semua endpoint dalam <span className="grad-text">satu platform</span></h2>
                        <p className="mt-2 text-slate-400">Terorganisir rapi & terdokumentasi otomatis.</p>
                    </Reveal>
                    <Stagger className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                        {CATS.map((c) => (
                            <StaggerItem key={c.name}>
                                <TiltCard className="glass noise h-full rounded-2xl p-6">
                                    <div className="grid h-11 w-11 place-items-center rounded-xl border border-white/10 bg-white/5 text-accent">
                                        <Icon name={c.icon} size={22} />
                                    </div>
                                    <h3 className="mt-3 text-lg font-semibold">{c.name}</h3>
                                    <p className="mt-1.5 text-sm text-slate-400">{c.desc}</p>
                                    <Link href="/docs" className="mt-4 inline-flex items-center gap-1 text-sm text-accent hover:underline">
                                        Explore <Icon name="arrow-right" size={14} />
                                    </Link>
                                </TiltCard>
                            </StaggerItem>
                        ))}
            ...
            <section className="px-4 py-16">
                <Reveal className="mx-auto max-w-4xl">
                    <div className="glass noise relative overflow-hidden rounded-[1.75rem] p-10 text-center md:p-14">
                        ...
                        <h2 className="text-3xl font-bold md:text-4xl">Mulai bangun dengan <span className="grad-text">StarNova API</span></h2>
